...
for key, value in odict.items():
	...
	while all(measurement_clipping_array_o < sigma + median) == "False"
		...
		for element in range(len(measurement_clipping_array_o)):

			if element > (sigma + median) or element < (sigma - median):

				measurement_clipping_array_o.remove(element)
				uncertainty_clipping_array_o.remove(element)
				time_clipping_array_o.remove(element)
