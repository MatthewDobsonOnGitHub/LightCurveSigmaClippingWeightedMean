...
# If a lone measurement occured on a day, its 'standard deviation' is assigned to be its raw uncertainty in flux.
...
# The code will ask you to enter the name of the transient that needs to be analysed.
# WARNING!!!!! Make sure that the forced photometry data file is called "(Transient Name)_raw_data.csv (see below for file reading), with "Transient Name" being exactly the same as what is entered into and read by the code via the keyboard.

supernova = input("Enter Transient to Analyse: ")
...
# For handling of the weighted data, arrays are defined for each variable to be used.
...
print("********** ORANGE ***********")
print()
r_time_o = []	# Raw Time (orange)
r_flux_o = []	# Raw Flux (orange)
r_dev_o = []	# Raw Error (orange)
r_mag_o = []	# Raw Magnitude (orange)
m_time_o = []	# Mean Time Values (orange)
w_flux_o = []	# Weighted Flux Values (orange)
s_dev_o = []	# Weighted Mean Standard Deviation values (orange)
num_o = []
print()

for key, value in odict.items():
    
	# Values and arrays used for each day are set to zero and blank arrays (respectively) for every 	day change.
	time_sum_o = 0.0
	...
	for row in value:
		...
		# Sums all times together for calculation of mean time
		time_sum_o += float(row["mjd"])  
 		# Part of weighted mean flux calculations
		flux_weight_product_sum_o += (float(row["calc_flux"])*float(row["weight"]))
		weight_sum_o += float(row["weight"])        
		# Appends values of flux, error and measurement time in a day to the 'per day' arrays 
	...
	print()	
	# calculation of mean time, weighted mean flux, the latter's standard deviation, and the number 	of data points that went into calculating the daily weighted mean.
...
plt.errorbar(r_time_o, r_flux_o, yerr=r_dev_o, fmt='o', color='orange', alpha = 0.4, capsize=5)
plt.errorbar(r_time_c, r_flux_c, yerr=r_dev_c, fmt='o', color='cyan', alpha = 0.4, capsize=5)
...
fig_name = directory + supernova + '_flux_vs_time.pdf'
