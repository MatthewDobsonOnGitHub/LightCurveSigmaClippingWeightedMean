...
for i in range(0, len(dates_o)):
	...
	for j in range(counter_o, len(time_o)):
		...
		else:	# This condition is when we have reached a measured time which occured the 				succeeding day.
...
plt.errorbar(time_mean_o, flux_mean_o, yerr = dflux_weighted_o, fmt = 'or', ecolor = 'r', capsize = 5)
plt.errorbar(time_mean_c, flux_mean_c, yerr = dflux_weighted_c, fmt = 'oc', ecolor = 'c', capsize = 5)
