...
for i in range(0, len(mag_c)):
	...
	if '>' not in mag_c[i]:
		...
		mag_string_c = str(mag_c[i])
		mag_correction_c.append(mag_string_c)


# Converting microjansky flux values to janskys

flux_jan_o = np.zeros(len(flux_o))

for i in range(0, len(flux_jan_o)):

	flux_jan_o[i] = flux_o[i]*10**(-6)

flux_jan_c = np.zeros(len(flux_c))

for i in range(0, len(flux_jan_c)):

	flux_jan_c[i] = flux_c[i]*10**(-6)

# Now we must calculate the magnitudes:

mag_recalc_o = np.zeros(len(flux_jan_o))
mag_recalc_c = np.zeros(len(flux_jan_c))

for i in range(0, len(mag_recalc_o)):

	if flux_jan_o[i] >= 0:

		mag_recalc_o[i] = (-2.5) * math.log(flux_jan_o[i], 10) + 8.9

for i in range(0, len(mag_recalc_c)):

	if flux_jan_c[i] >= 0:

		mag_recalc_c[i] = (-2.5) * math.log(flux_jan_c[i], 10) + 8.9
