...
y = np.zeros(len(y_raw))	# Can't call it as empty array as it must be same length as y_raw.

_list_ = []	# Create an empty array for analysing the y_raw and y_values.

for element in range(0, len(y_raw)):
	
	_list_.append( y_raw[element] )
	

	if _list_[element] == 'nan':
