...
# in case the filename needs to be changed, it's more accessible to define it here as a string variable.	

filename = "SN2018hmx_raw_data.csv" 
...
filename_2 = 'SN2019ehk_flux_raw_weighted.txt'

data_raw_o = np.array([time_o, flux_o, dflux_o])
data_raw_c = np.array([time_c, flux_c, dflux_c])
data_wei_o = np.array([time_mean_o, flux_weighted_mean_o, stdev_o])
data_wei_c = np.array([time_mean_o, flux_weighted_mean_o, stdev_o])
...
# here, you open the ascii file
	...
	np.savetxt(datafile_id, data_raw_o, fmt=['%s','%s','%s'], delimiter='\t\t', header='Time, Flux and Flux Error for Orange Filter (Raw data)', footer='\n')
	np.savetxt(datafile_id, data_raw_c, fmt=['%s','%s','%s'], delimiter='\t\t', header='Time, Flux and Flux Error for Cyan Filter (Raw data)', footer='\n')
	np.savetxt(datafile_id, data_wei_o, fmt=['%s','%s','%s'], delimiter='\t\t', header='Time, Flux and Flux Error for Orange Filter (Weighted Averages)', footer='\n')
	np.savetxt(datafile_id, data_wei_c, fmt=['%s','%s','%s'], delimiter='\t\t', header='Time, Flux and Flux Error for Cyan Filter (Weighted Averages)', footer='\n')
...
plt.errorbar(time_mean_o, flux_weighted_mean_o, yerr=stdev_o, fmt='o', color='orange', capsize=5)
plt.errorbar(time_mean_c, flux_weighted_mean_c, yerr=stdev_c, fmt='o', color='cyan', capsize=5)
...
plt.savefig('SN2018hmx_flux_vs_time_raw_and_weighted_mean.pdf')
