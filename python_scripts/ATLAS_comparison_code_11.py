...
filename = supernova + "_raw_data_trunc.csv"
...
for i in range(0, len(r_time_o_hawaii)):
	
	tDiff_o = (r_time_o_hawaii[i] - r_time_o[i])
	time_difference_o.append(tDiff_o)
	fDiff_o = (r_flux_o_hawaii[i] - r_flux_o[i])
	flux_difference_o.append(fDiff_o)

for i in range(0, len(r_time_c_hawaii)):
	
	tDiff_c = (r_time_c_hawaii[i] - r_time_c[i])
	time_difference_c.append(tDiff_c)
	fDiff_c = (r_flux_c_hawaii[i] - r_flux_c[i])
	flux_difference_c.append(fDiff_c)
	
print(len(r_flux_o))
print(len(r_flux_o_hawaii))
print(len(flux_difference_o))
print(len(r_flux_c))
print(len(r_flux_c_hawaii))
print(len(flux_difference_c))
...
filename_2 = '/home/mdobson/Archive/Objects/' + supernova + '/ATLAS/' + supernova + '_flux_weighted_mean_comparison_with_hawaii.txt'
...
plt.figure("Flux vs Time", figsize=(8,8))

plt.subplots_adjust(hspace=1, wspace=0)

plt.subplot(311)
plt.subplot(311).set_title("Flux vs Time from Usual Method")
...
plt.subplot(312)
plt.subplot(312).set_title("Flux vs Time from Alternate Method")
...
plt.subplot(313)
plt.subplot(313).set_title("Difference in Flux Values from Both Methods vs Normal Flux Values")

plt.plot(raw_data_o["Flux"], flux_difference_o,'o', color='orange')
plt.plot(raw_data_c["Flux"], flux_difference_c,'o', color='cyan')

# Setting the titles for the x- and y-axes.
# x-axis is time in units if Modified Julian Date.
# y-axis is magnitude of transient's light.

plt.xlabel('Normal Flux')
...
fig_name = supernova + '_flux_vs_time_ATLAS_comparison_with_hawaii.pdf'
