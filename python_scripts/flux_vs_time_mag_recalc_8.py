...
mag_3_sigma_o = np.zeros(len(dmagnitude_recalc_o))
mag_3_sigma_c = np.zeros(len(dmagnitude_recalc_c))

for i in range(0, len(dmagnitude_recalc_o)):
	mag_3_sigma_o[i] = dmagnitude_recalc_o[i] * 3
for i in range(0, len(dmagnitude_recalc_c)):
	mag_3_sigma_c[i] = dmagnitude_recalc_c[i] * 3
...
data_mag_o = np.array([time_corresponding_o, flux_corresponding_o, magnitude_recalc_o, dmagnitude_recalc_o, mag_3_sigma_o])
data_mag_c = np.array([time_corresponding_c, flux_corresponding_c, magnitude_recalc_c, dmagnitude_recalc_c, mag_3_sigma_c])
...
# here, you open the ascii file
	...
	np.savetxt(datafile_id, data_mag_o, fmt=['%s','%s','%s','%s','%s'], delimiter='\t\t', header='Time (MJD), Flux (microjanskys) and Calculated Magnitude Value and 3-sigma Error for Orange Filter (Raw data)',footer='\n\n\n')
	np.savetxt(datafile_id, data_mag_c, fmt=['%s','%s','%s','%s','%s'], delimiter='\t\t', header='Time (MJD), Flux (microjanskys) and Calculated Magnitude Value and 3-sigma Error for Cyan Filter (Raw data)',footer='\n\n\n')
...
plt.errorbar(r_time_o, r_flux_o, yerr=r_dev_o, fmt='o', color='orange', alpha = 0.2, capsize=5)
plt.errorbar(time_corresponding_c, magnitude_recalc_c, yerr=r_dev_c, fmt='o', color='cyan', alpha = 0.2, capsize=5)
