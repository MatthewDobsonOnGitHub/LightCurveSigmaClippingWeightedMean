...
def general_code():
	...
	# Because these varibales are used over many functions, but alos because they are defined in a functon, they have to be defined as a global variable, meaning they can be passed across all the functions.
	# However, this is risky and bad practice to have global variables, as it might be changed by accident or error and cause mayhem all over!
	# Alternatively, one could modify the functions to include 'determiner' and 'number_sigma' as argument parameters in their inputs!
	global determiner
	global number_sigma
	global filename
	...
	# Thus, depending on which file is used, the code must do different things.

	determiner = int(input("There are two data file formats you can input to this file: \n1. Forced photometry from ATLAS website \n2. Forced Photometry output from forced.sh code. \nIf you wish to use the former, enter 1 into the terminal. \nIf you wish to use the latter, enter 2 into the terminal.\n"))
	...
	if determiner == 1:
		...
		keyboardInput = input("Please enter name of the data file output from the forced.sh code:\t")
