...
########################### CROSS-CHECKING MAGNITUDE CALCULATIONS ###########################

# This is a way to cross-check the flux calculations. We convert the calculated flux values into janskys, then into magnitudes, and then plot them against the raw magnitude values in the original datafile.
...
# Since we don't know how many magnitude values will be sifted and calculated without looking at the raw dat afile, we define arrays that will contain intermediate calculated quantities as empty, and append values to them.
...
# Ensures that a given flux value is not a limit, and is positive (as the logarithm function cannot take negative arguments

for i in range(0, len(mag_o)):

	if '>' not in mag_o[i] and flux_o[i] >= 0:

		mag_string_o = float(mag_o[i])
		mag_correction_o.append(mag_string_o)
		flux_for_conv_o.append(flux_o[i])	

for i in range(0, len(mag_c)):

	if '>' not in mag_c[i] and flux_c[i] >= 0:

		mag_string_c = float(mag_c[i])
		mag_correction_c.append(mag_string_c)
		flux_for_conv_c.append(flux_c[i])


# Converting microjansky flux values to janskys

flux_jan_o = np.zeros(len(flux_for_conv_o))

for i in range(0, len(flux_jan_o)):

	flux_jan_o[i] = flux_for_conv_o[i]*10**(-6)

flux_jan_c = np.zeros(len(flux_for_conv_c))

for i in range(0, len(flux_jan_c)):

	flux_jan_c[i] = flux_for_conv_c[i]*10**(-6)
...
for i in range(0, len(flux_jan_o)):
	...
	if flux_jan_o[i] >= 0:
		...
		mag_calc_o = (-2.5) * math.log(flux_jan_o[i], 10) + 8.9
		mag_recalc_o.append(mag_calc_o)

for i in range(0, len(flux_jan_c)):

	if flux_jan_c[i] >= 0:

		mag_calc_c = (-2.5) * math.log(flux_jan_c[i], 10) + 8.9
		mag_recalc_c.append(mag_calc_c)

print()
print("Measured magnitudes - orange")
print(mag_correction_o)
print()
print("Re-calculated magnitudes - orange")
print(mag_recalc_o)
print()
print("Measured magnitudes - cyan")
print(mag_correction_c)
print()
...
print()
print("Measured magnitudes - orange")
print(len(mag_correction_o))
print()
print("Re-calculated magnitudes - orange")
print(len(mag_recalc_o))
print()
print("Measured magnitudes - cyan")
print(len(mag_correction_c))
print()
print("Re-calculated magnitudes - cyan")
print(len(mag_recalc_c))
print()
print()
print()		
...
for i in range(0, len(mag_recalc_o)):
	mag_diff_for_o = mag_recalc_o[i] - mag_correction_o[i]
	mag_diff_o.append(mag_diff_for_o)

for i in range(0, len(mag_recalc_c)):
	mag_diff_for_c = mag_recalc_c[i] - mag_correction_c[i]
...
data_raw_o = np.array([time_o, flux_o, dflux_o])
data_raw_c = np.array([time_c, flux_c, dflux_c])
...
# Now, as a useful sanity check, to plot the difference between the measured and recalculated magnitude values
...
plt.plot(mag_correction_o, mag_diff_o, 'o', color='magenta')
plt.plot(mag_correction_c, mag_diff_c, 'o', color='magenta')
...
fig_name = supernova + '_flux_vs_time_raw_and_weighted_mean_with_check.pdf'
