...
def general_code1(filename, telescope, firstLine):
	...
	for row in reader:
		...
		if row["FILTER"] == 'o':
			...
			# This appends the data of a single measurement to the dictionary appropriate to the filter that measurement was taken on. 
				odict[row["MJD_OBS"]].append(dict(row))
		...
		if row["FILTER"] == 'c':
			...
			try:
			# This appends the data of a single measurement to the dictionary appropriate to the filter that measurement was taken on. 
				cdict[row["MJD_OBS"]].append(dict(row))
	...
	for key, value in odict.items():
		...
		for row in value:
			...
			time_o.append(float(row["MJD_OBS"]))
	...
	for key, value in cdict.items():
		...
		for row in value:
			...
			# Appends values to appropriate arrays
			time_c.append(float(row["MJD_OBS"]))
	...
	fig_name = telescope + '_throughput_(magzpt)_vs_time.png'
...
def general_code2(filename, telescope, firstLine):
	...
	for row in reader:
		...
		if "MJD_OBS" in firstLine:
			row["MJD_OBS"] = row["MJD_OBS"].strip()
		elif "MJD_OBS" in firstLine:
			row["MJD_OBS"] = row["MJD_OBS"].strip()
	...
	fig_name = telescope + '_throughput_(mag5sig)_vs_time.png'
...
def general_code3(filename, telescope, firstLine):
	...
	plt.figure("Throughput (Seeing) vs Time - " + telescope + " Full Time")
	...
	fig_name = telescope + '_throughput_(seeing)_vs_time.png'
...
# The number of arguments to the code can be set to any given number; we set it to 1, as only one file need be entered at a time.
	...
	if confirm == 'y':
		...
		if measure == '1':
			...
			general_code1(filename, telescope, firstLine)

		if measure == '2':

			general_code2(filename, telescope, firstLine)

		if measure == '3':

			general_code3(filename, telescope, firstLine)
