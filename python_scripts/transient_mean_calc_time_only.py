...
divisor = 0		# How many values are being averaged in a given binned day
...
for i in range(0, len(floored_time_unique)):	# Runs through all the floored time values (i.e. the MJD 							  integer date values)
	...
	for j in range(counter, len(time_raw)):
		...
		if math.floor(time_raw[j]) == floored_time_unique[i]:
			...
			mean_time_calculated = cumulative_time_sum / divisor


			# ...and the mean magnitude...
			#mean_mag_calculated = cumulative_mag_sum / divisor
...
print(len(time_raw))		# checking dimensions of the arrays.
