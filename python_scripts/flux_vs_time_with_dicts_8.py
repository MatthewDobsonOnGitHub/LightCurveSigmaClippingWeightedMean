...
for key, value in odict.items():
	...
	stdv_o = np.std(measurement_array_o, ddof=1)
...
for key, value in cdict.items():
	...
	stdv_c = np.std(measurement_array_c, ddof=1)
...
no_of_measurements_o = np.ones(len(mean_data_o))
no_of_measurements_c = np.ones(len(mean_data_c))


filename_2 = supernova + '_flux_raw_weighted_dicts.txt'

data_raw_o = np.array([time_array_o, measurement_array_o, uncertainty_array_o])
data_raw_c = np.array([time_array_c, measurement_array_c, uncertainty_array_c])
data_wei_o = np.array([np.array(mean_data_o["Time"]), np.array(mean_data_o["Flux"]), np.array(mean_data_o["Error"]), no_of_measurements_o])
data_wei_c = np.array([np.array(mean_data_c["Time"]), np.array(mean_data_c["Flux"]), np.array(mean_data_c["Error"]), no_of_measurements_c])
...
#	np.savetxt(datafile_id, data_wei_o, fmt=['%s','%s','%s'], delimiter='\t\t', header='Time (MJD), Flux, Flux Error (microjanskys)', footer='\n')
#	np.savetxt(datafile_id, data_wei_c, fmt=['%s','%s','%s'], delimiter='\t\t', header='Time (MJD), Flux, Flux Error (microjanskys)', footer='\n')
...
plt.errorbar(mean_data_o["Time"], mean_data_o["Flux"], yerr=mean_data_o["Uncertainty"], fmt='o', color='red', capsize=5)
plt.errorbar(mean_data_c["Time"], mean_data_c["Flux"], yerr=mean_data_c["Uncertainty"], fmt='o', color='blue', capsize=5)
...
fig_name = supernova + '_flux_vs_time_raw_and_weighted_mean_dict_auto_sigma.pdf'
