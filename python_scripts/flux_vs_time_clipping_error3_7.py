...
for key, value in clipped_data_o.items():
	...
	for row in value:
		...
		uncertainty_array_o.append(row["calc_dflux"]) 
		time_array_o.append(row["mjd"]) 
		c_r_flux_o.append(float(row["calc_flux"])) 
		c_r_dev_o.append(float(row["calc_dflux"])) 
		c_r_time_o.append(float(row["mjd"])) 
	...
	c_m_time_o.append(mean_time_o)
	c_w_flux_o.append(weighted_flux_o)
	c_s_dev_o.append(stdv_o)
	c_num_o.append(len(value))
	...
	if len(value) > 1:
		...
		weighted_variance_o = sqdiff_o/(ratio_o*weight_sum_o)
		weighted_standard_deviation_o = weighted_variance_o**(1/2) 
		c_new_error_o.append(weighted_standard_deviation_o)

	elif len(value) == 1:

		stdv_o = standard_deviation(measurement_array_o, uncertainty_array_o)
		c_new_error_o.append(stdv_o)
...
clipped_mean_data_o["Time"] = c_m_time_o
clipped_mean_data_o["Flux"] = c_w_flux_o
clipped_mean_data_o["STDEV"] = c_s_dev_o
clipped_mean_data_o["Number"] = c_num_o
clipped_mean_data_o["Error"] = c_new_error_o
clipped_raw_data_o["Time"] = c_r_time_o
clipped_raw_data_o["Flux"] = c_r_flux_o
clipped_raw_data_o["STDEV"] = c_r_dev_o
...
for key, value in clipping_data_c.items():
	...
	num_c.append(len(value))

	if len(value) > 1:

		for row in value:

			sqdiff_c += float(row["weight"])*(weighted_flux_c - float(row["calc_flux"]))**2

		ratio_c = (len(value)-1)/ len(value)
		weighted_variance_c = sqdiff_c/(ratio_c*weight_sum_c)
		weighted_standard_deviation_c = weighted_variance_c**(1/2) 
		new_error_c.append(weighted_standard_deviation_c)

	elif len(value) == 1:

		stdv_c = standard_deviation(measurement_array_c, uncertainty_array_c) 
		new_error_c.append(stdv_c)
...
for key, value in clipped_data_c.items():
	...
	for row in value:
		...
		uncertainty_array_c.append(row["calc_dflux"]) 
		time_array_c.append(row["mjd"]) 
		c_r_flux_c.append(float(row["calc_flux"])) 
		c_r_dev_c.append(float(row["calc_dflux"])) 
		c_r_time_c.append(float(row["mjd"])) 
	...
	c_m_time_c.append(mean_time_c)
	c_w_flux_c.append(weighted_flux_c)
	c_s_dev_c.append(stdv_c)
	c_num_c.append(len(value))
	...
	if len(value) > 1:
		...
		weighted_variance_c = sqdiff_c/(ratio_c*weight_sum_c)
		weighted_standard_deviation_c = weighted_variance_c**(1/2) 
		c_new_error_c.append(weighted_standard_deviation_c)

	elif len(value) == 1:

		stdv_c = standard_deviation(measurement_array_c, uncertainty_array_c) 
		c_new_error_c.append(stdv_c)
...
clipped_mean_data_c["Time"] = c_m_time_c
clipped_mean_data_c["Flux"] = c_w_flux_c
clipped_mean_data_c["STDEV"] = c_s_dev_c
clipped_mean_data_c["Number"] = c_num_c
clipped_mean_data_c["Error"] = c_new_error_c
clipped_raw_data_c["Time"] = c_r_time_c
clipped_raw_data_c["Flux"] = c_r_flux_c
clipped_raw_data_c["STDEV"] = c_r_dev_c
