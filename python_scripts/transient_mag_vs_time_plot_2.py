...
for i in range(0, len(floored_time_unique)):	
	...
	for j in range(counter, len(time_raw)):
		...
		if math.floor(time_raw[j]) == floored_time_unique[i] and math.isnan(mag_raw[j]) != True:
			...
			cumulative_mag_sum = cumulative_mag_sum + mag_raw[j] 											  
			...
			# Cumulative sum of magnitude values
			#cumulative_mag_sum = cumulative_mag_sum 											  

			# Keeps count of number of time values that have been summed thus far
			time_divisor = time_divisor + 1		
	
			# Even though we are not counting 'NaN' mag values, making the mag_divisor equal 				  1 prevents division by zero.
			# mag_divisor = 1
...
# Remembering that the more negative the magnitude the brighter the transient, so we must reverse the direction of the y-axis.
...
plt.savefig('mag_vs_time_plot.pdf')
