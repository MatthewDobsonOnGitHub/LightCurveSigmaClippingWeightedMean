...
filename_PANSTARRS = supernova + "_raw_data.csv"
filename_ATLAS = supernova + ".csv"

# each row in the datafile becomes its own row; 'reader' contains many dictionaries. Each dictionary is an 'ordered dictionary'.

reader_PANSTARRS = csv.DictReader(open(filename_PANSTARRS))
reader_ATLAS = csv.DictReader(open(filename_ATLAS))
	...
	if row["mjd"] != 'None' and row["peakfit"] != 'None' and row["major"] != 'None' and row["minor"] != 'None' and row["zp"] != 'None' and row["apfit"] != 'None':
		...
		if row["filter"] == 'o':
			...
			# The syntax here is: dictionary["new_key"].append(array)
				...
				odict_PANSTARRS[int(float(row["mjd"]))].append(dict(row))
			...
			# However, if this key does not exist, we need to create this key:value pair to begin with, creating it as an empty list. However, the following code must be kept separate, and run only once per measurement day analysed, as repeating this will erase all rows of data apart from the last one in the day.
				...
				odict_PANSTARRS[int(float(row["mjd"]))] = []
				odict_PANSTARRS[int(float(row["mjd"]))].append(dict(row))
		...
		# If the measurement was made with the cyan filter
			...
			try:
				...
				cdict_PANSTARRS[int(float(row["mjd"]))].append(dict(row))
            
			# However, if this key does not exist, we need to create this key:value pair to begin with, creating it as an empty list. However, the following code must be kept separate, and run only once per measurement day analysed, as repeating this will erase all rows of data apart from the last one in the day.
            
			except KeyError as e:
                
				cdict_PANSTARRS[int(float(row["mjd"]))] = []
				cdict_PANSTARRS[int(float(row["mjd"]))].append(dict(row))

for row in reader_ATLAS:
...
for key, value in odict_PANSTARRS.items():
    
	time_sum_o_pan = 0.0
	flux_weight_product_sum_o_pan = 0.0
	weight_sum_o_pan = 0.0
	stdv_o_pan = 0.0
	print()
	print("Day MJD")
	print(key)
	measurement_array_o_pan = []  
	uncertainty_array_o_pan = []
	time_array_o_pan = []

	for row in value:

		print(row["mjd"], row["calc_flux"], row["calc_dflux"], row["weight"])   
		time_sum_o_pan = time_sum_o_pan + float(row["mjd"])   
		flux_weight_product_sum_o_pan = flux_weight_product_sum_o_pan + (float(row["calc_flux"])*float(row["weight"]))
		weight_sum_o_pan = weight_sum_o_pan + float(row["weight"])         
		measurement_array_o_pan.append(row["calc_flux"]) 
		uncertainty_array_o_pan.append(row["calc_dflux"]) 
		time_array_o_pan.append(row["mjd"]) 

		r_flux_o_pan.append(float(row["calc_flux"])) 
		r_dev_o_pan.append(float(row["calc_dflux"])) 
		r_time_o_pan.append(float(row["mjd"])) 

	# The number of times summed is the length of the 'value' array.  
	print()	
	mean_time_o_pan = time_sum_o_pan / len(value)
	weighted_flux_o_pan = flux_weight_product_sum_o_pan / weight_sum_o_pan
	stdv_o = standard_deviation(measurement_array_o_pan, uncertainty_array_o_pan)
	print("Mean Time Sum - Cyan")
	print(mean_time_o_pan)
	m_time_o.append(mean_time_o_pan)
	print()
	print("Weighted Sum - Cyan")
	print(weighted_flux_o)
	w_flux_o.append(weighted_flux_o_pan)
	print()
	print("Uncertainty Associated with Weighted Mean - Orange")
	print(stdv_o_pan)
	s_dev_o.append(stdv_o_pan)
	print()
	print("Number of Data Points Averaged in this Day - Orange")
	print(len(value))
	number_o_pan = len(value)
	num_o_pan.append(number_o_pan)
	print()
	
mean_data_o_pan["Time"] = m_time_o_pan
mean_data_o_pan["Flux"] = w_flux_o_pan
mean_data_o_pan["Error"] = s_dev_o_pan
mean_data_o_pan["Number"] = num_o_pan

raw_data_o_pan["Time"] = r_time_o_pan
raw_data_o_pan["Flux"] = r_flux_o_pan
raw_data_o_pan["Error"] = r_dev_o_pan
...
print()
for key, value in cdict_PANSTARRS.items():
    
	time_sum_c_pan = 0.0
	flux_weight_product_sum_c_pan = 0.0
	weight_sum_c_pan = 0.0
	stdv_c_pan = 0.0
	print()
	print("Day MJD")
	print(key)
	measurement_array_c_pan = []  
	uncertainty_array_c_pan = []
	time_array_c_pan = [] 

	for row in value:

		print(row["mjd"], row["calc_flux"], row["calc_dflux"], row["weight"])   
		time_sum_c_pan = time_sum_c_pan + float(row["mjd"])   
		flux_weight_product_sum_c_pan = flux_weight_product_sum_c_pan + (float(row["calc_flux"])*float(row["weight"]))
		weight_sum_c_pan = weight_sum_c_pan + float(row["weight"])         
		measurement_array_c_pan.append(row["calc_flux"]) 
		uncertainty_array_c_pan.append(row["calc_dflux"]) 
		time_array_c_pan.append(row["mjd"]) 

		r_flux_c_pan.append(float(row["calc_flux"])) 
		r_dev_c_pan.append(float(row["calc_dflux"])) 
		r_time_c_pan.append(float(row["mjd"])) 


	# The number of times summed is the length of the 'value' array.  
	print()	
	mean_time_c_pan = time_sum_c_pan / len(value)
	weighted_flux_c_pan = flux_weight_product_sum_c_pan / weight_sum_c_pan
	stdv_c_pan = standard_deviation(measurement_array_c_pan, uncertainty_array_c_pan)
	print("Mean Time Sum - Cyan")
	print(mean_time_c_pan)
	m_time_c_pan.append(mean_time_c_pan)
	print()
	print("Weighted Sum - Cyan")
	print(weighted_flux_c_pan)
	w_flux_c_pan.append(weighted_flux_c_pan)
	print()
	print("Uncertainty Associated with Weighted Mean - Cyan")
	print(stdv_c_pan)
	s_dev_c.append(stdv_c_pan)
	print()
	print("Number of Data Points Averaged in this Day - Cyan")
	print(len(value))
	number_c_pan = len(value)
	num_c_pan.append(number_c_pan)
	print()

mean_data_c_pan["Time"] = m_time_c_pan
mean_data_c_pan["Flux"] = w_flux_c_pan
mean_data_c_pan["Error"] = s_dev_c_pan
mean_data_c_pan["Number"] = num_c_pan

raw_data_c_pan["Time"] = r_time_c_pan
raw_data_c_pan["Flux"] = r_flux_c_pan
raw_data_c_pan["Error"] = r_dev_c_pan
