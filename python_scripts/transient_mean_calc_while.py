...
# For every new date, the variables used in the calculations are reset.
	...
	while math.floor(time_raw[i]) == floored_time_values[i]:


		cumulative_time_sum = cumulative_time_sum + time_raw[i]	# Cumulative sum of time 											  values
