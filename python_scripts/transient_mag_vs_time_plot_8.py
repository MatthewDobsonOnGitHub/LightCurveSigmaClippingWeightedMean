...
# The 'NaN' entries for magnitude measurements that are only upper limits cause problems with the summation. Therefore, we must define two conditions, based on whether or not the magnitude measurement is a known value or merely an upper limit.
		...
		if math.floor(time_raw[j]) == floored_time_unique[i] and math.isnan(mag_raw[j]) != True:
			...
			# We do not add such magnitude values to the total if this condition is true.
			#cumulative_mag_sum += mag_raw[j] 
			...
			# This takes into account the case where a single upper-limit measurement 				occured on a given day, and prevents division by zero.
			if mag_divisor == 0:
				mag_divisor = 1
...
# The mean-value arrays still contain the upper limit measurements, either as 'NaN' or magnitude = 0.0. One can use either of the two loops below to filter out these values.
...
#	if math.isnan(mag_mean[i]) != True:
#		time_mean_final.append(time_mean[i])
#		mag_mean_final.append(mag_mean[i])
#	else:
#		continue

#Use below code if magnitude data contains '0.0' for an upper limit
for i in range(0, len(mag_mean)):
	if mag_mean[i] != 0.0:
		time_mean_final.append(time_mean[i])
		mag_mean_final.append(mag_mean[i])
