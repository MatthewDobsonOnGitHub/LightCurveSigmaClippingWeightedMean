#!/usr/bin/env python
# THE ABOVE LINE OF CODE MUST BE INCLUDED FOR THIS SCRIPT TO BE READ TO THE COMMAND LINE!
#
#
...
mag_imported = data[:,3]		# Raw Magnitude Values
...
# The data imported as it is will contain magnitude measurements that are defined only by their lower limit (i.e. their upper limit fo brightness) -- this manifests as a 'greater than' sign, which the reading function cannot handle and imports the entry as 'NaN'. Since these values are not necessarily needed, we will remove them from the arrays and replace them with empty items.

mag_string = []


for i in range(0, len(mag_imported)):

	mag_str = str(mag_imported[i])
	mag_string.append(mag_str)
...
for i in range(0, len(mag_string)):

	mag_r = float(mag_string[i])
	mag_raw.append(mag_r)
...
time_divisor = 0		# How many values of time are being averaged in a given binned day

mag_divisor = 0			# How many values of magnitude are being averaged on a given binned day
...
for i in range(0, len(floored_time_unique)):	# Runs through all the floored time values (i.e. the MJD 							  integer date values)
	...
	for j in range(counter, len(time_raw)):
		...
		if math.floor(time_raw[j]) == floored_time_unique[i]:
			...
			cumulative_mag_cum = cumulative_mag_sum + mag_raw[j] 											  

			# Keeps count of number of time values that have been summed thus far
			time_divisor = time_divisor + 1	

			
			# If the magnitude measurement is not an upper limit
			if mag_raw[j] != 0:

				# Keeps count of number of magnitude values that have been summed thus 					  far	
...
# However, we do not want the inner nested loop to restart; we want it to continue along the list fo raw time values; we just want the index of the outer loop (for the date values) to increase by 1. To do this, we use a counter that increases by 1 every time we move along an array. When a day si finished averaging, the loop breaks, then returns to that starting point in a range governed by 'counter'
...
print(len(time_mean))		# this should be equal to just below:
print(len(mag_mean))
...
plt.plot(time_mean, mag_mean, marker = 'o', capsize = 5, linestyle = '')
