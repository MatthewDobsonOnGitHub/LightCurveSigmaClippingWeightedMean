...
time_divisor = 0		# How many values are being averaged in a given binned day
...
for i in range(0, len(floored_time_unique)):	
	...
	for j in range(counter, len(time_raw)):
		...
		if math.floor(time_raw[j]) == floored_time_unique[i] and mag_raw[j] != 'nan':
		...
		elif math.floor(time_raw[j]) == floored_time_unique[i] and mag_raw[j] == 'nan':
