...
def general_code(filename, telescope, firstLine, throughput, timeKey, delimitingcharacter):
	...
	reader = csv.DictReader(open(filename), delimiter=delimitingcharacter, skipinitialspace=True)
...
# The number of arguments to the code can be set to any given number; we set it to 1, as only one file need be entered at a time.
	usage = "Usage: %s <SN filename> <SN filename>" % argv[0]
	if len(argv) != 3:
		sys.exit(usage)
	...
	if confirm == 'y':
		...
		if measure == '1':
			...
			throughput = "MAGZPT"
			general_code(filename, telescope, firstLine, throughput, timeKey, delimitingcharacter)
