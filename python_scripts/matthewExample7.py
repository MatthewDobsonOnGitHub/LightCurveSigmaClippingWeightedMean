...
for key, value in odict.items():
	...
	interval = abs(med_o - stdv_o)
	
	for row in value:

		if abs(med_o - row["calc_flux"]) > interval:
	...
	print("Clipped values (1 iteration)")
...
for key, value in cdict.items():
	...
	interval = abs(med_c - stdv_c)
	
	for row in value:

		if abs(med_c - row["calc_flux"]) > interval:
