...
# csv.DictReader is much better better than the np.genfromtxt - it imports the data in a file as a list of dictionaries; given a table of data in columns, the column header becomes the key of a dictionary, and the value in that column for a given row the key's value for a given dictionary (that corresponds to said row).

#		key 1		key 2		key 3
# dict 1	value 1.1	value 1.2	value 1.3
# dict 2	value 2.1	value 2.2	value 2.3 
# dict 3	value 3.1	value 3.2	value 3.3 
...
# Calculating the numerator of the standard deviation expression inside the square root
...
for i in range(0, len(dates_o)):
	...
	for j in range(counter_o, len(time_o)):
		...
		if math.floor(time_o[j]) == dates_o[i]: #and denominators_o != 0:
		...
		#elif math.floor(time_o[j]) == dates_o[i] and denominators_o == 0:
...
plt.savefig('flux_vs_time_mean_plot_stdev_and_errors_clipped_3.pdf')
