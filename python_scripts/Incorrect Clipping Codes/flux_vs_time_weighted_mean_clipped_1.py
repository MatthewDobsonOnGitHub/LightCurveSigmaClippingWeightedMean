...
# Now to calculate the standard deviation of the weighted mean values. According to the Least Mean Square method, this is equal to the square root of the inverse of the summed weights for a given day.
...
# Some data points will have very large uncertainties, both in absolute terms and compared to others.
# These can be removed from the plot and code without significant loss of accuracy.
...
flux_weighted_mean_clipped_o = []
flux_weighted_mean_clipped_c = []
flux_weighted_mean_clipped_err_o = []
flux_weighted_mean_clipped_err_c = []
time_mean_clipped_o = []
time_mean_clipped_c = []


for i in range(0, len(flux_weighted_mean_err_o)):

	if flux_weighted_mean_err_o[i] <= 50:

		flux_weighted_mean_clipped_o.append(flux_weighted_mean_o[i])
		time_mean_clipped_o.append(time_mean_o[i])
		flux_weighted_mean_clipped_err_o.append(flux_weighted_mean_err_o[i])

for i in range(0, len(flux_weighted_mean_err_c)):

	if flux_weighted_mean_err_c[i] <= 50:

		flux_weighted_mean_clipped_c.append(flux_weighted_mean_c[i])
		time_mean_clipped_c.append(time_mean_c[i])
		flux_weighted_mean_clipped_err_c.append(flux_weighted_mean_err_c[i])	

print()
print("Orange Flux Length")
print(len(flux_weighted_mean_o))
print("Clipped Orange Flux Length")
print(len(flux_weighted_mean_clipped_o))
print("Orange Time Length")
print(len(time_mean_clipped_o))
print("Orange Uncertainties")
print(len(flux_weighted_mean_clipped_err_o))
print("Cyan Flux Length")
print(len(flux_weighted_mean_c))
print("Clipped Orange Flux Length")
print(len(flux_weighted_mean_clipped_c))
print("Cyan Time Length")
print(len(time_mean_clipped_c))
print("Cyan Uncertainties Length")
print(len(flux_weighted_mean_clipped_err_c))
print()
print()
print("Orange Uncertainties")
print(flux_weighted_mean_clipped_err_o)
print()
print("Cyan Uncertainties")
print(flux_weighted_mean_clipped_err_c)
...
plt.errorbar(time_mean_clipped_o, flux_weighted_mean_clipped_o, yerr=flux_weighted_mean_clipped_err_o, fmt='o', color='red', capsize=5)
plt.errorbar(time_mean_clipped_c, flux_weighted_mean_clipped_c, yerr=flux_weighted_mean_clipped_err_c, fmt='o', color='blue', capsize=5)

#plt.plot(time_mean_o, flux_mean_o, 'o', color='magenta')
#plt.plot(time_mean_c, flux_mean_c, 'o', color='green')
...
plt.savefig('flux_vs_time_weighted_mean_plot_clipping.pdf')
