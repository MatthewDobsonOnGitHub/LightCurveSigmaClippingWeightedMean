...
dmagnitude_recalc_o = []	# recalculated magnitude values (orange - all)
dmagnitude_recalc_c = []	# recalculated magnitude values (cyan - all)
flux_corresponding_o = [] 
flux_corresponding_c = []
dflux_corresponding_o = [] 
dflux_corresponding_c = []
time_corresponding_o = []
time_corresponding_c = []


for i in range(0, len(r_mag_o)):
	if '>' not in r_mag_o[i] and r_flux_o[i] >= 0:
...
for i in range(0, len(r_mag_c)):
	if '>' not in r_mag_c[i] and r_flux_c[i] >= 0:
...
dflux_jan_o = np.zeros(len(flux_corresponding_o))
...
dflux_jan_c = np.zeros(len(flux_corresponding_c))

for i in range(0, len(flux_jan_c)):
	flux_jan_c[i] = flux_corresponding_c[i]*10**(-6)
	dflux_jan_c[i] = dflux_corresponding_c[i]*10**(-6)
...
data_raw_o = np.array([raw_data_o["Time"], raw_data_o["Flux"], raw_data_o["Error"], raw_data_o["Flux"], dmagnitude_recalc_o])
data_raw_c = np.array([raw_data_c["Time"], raw_data_c["Flux"], raw_data_c["Error"], raw_data_o["Flux"], dmagnitude_recalc_c])
...
data_mag_o = np.array([time_corresponding_o, flux_corresponding_o, magnitude_recalc_o])
data_mag_c = np.array([time_corresponding_c, flux_corresponding_c, magnitude_recalc_c])
...
# here, you open the ascii file
	...
	np.savetxt(datafile_id, data_raw_o, fmt=['%s','%s','%s','%s','%s'], delimiter='\t\t', header='Time (MJD), Flux (microJy), Flux Error (microJy), Magnitude and Mag Error for Orange Filter (Raw data)',footer='\n')
	np.savetxt(datafile_id, data_raw_c, fmt=['%s','%s','%s','%s','%s'], delimiter='\t\t', header='Time (MJD), Flux (microJy), Flux Error (microJy), Magnitude and Mag Error for Cyan Filter (Raw data)',footer='\n')
	...
	np.savetxt(datafile_id, data_raw_o, fmt=['%s','%s','%s'], delimiter='\t\t', header='Time (MJD), Flux (microjanskys) and Calculated Magnitude for Orange Filter (Raw data)',footer='\n')
	np.savetxt(datafile_id, data_raw_c, fmt=['%s','%s','%s'], delimiter='\t\t', header='Time (MJD), Flux (microjanskys) and Calculated Magnitude for Cyan Filter (Raw data)',footer='\n')
