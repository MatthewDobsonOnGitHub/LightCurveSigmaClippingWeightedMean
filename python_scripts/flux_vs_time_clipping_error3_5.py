...
for key, value in clipped_data_o.items():
	...
	for row in value:
		...
		c_flux_o.append(float(row["calc_flux"])) 
		c_dev_o.append(float(row["calc_dflux"])) 
		c_time_o.append(float(row["mjd"])) 
...
clipped_data_o["Time"] = c_time_o
clipped_data_o["Flux"] = c_flux_o
clipped_data_o["STDEV"] = c_dev_o
...
for key, value in clipped_data_c.items():
	...
	for row in value:
		...
		c_flux_c.append(float(row["calc_flux"])) 
		c_dev_c.append(float(row["calc_dflux"])) 
		c_time_c.append(float(row["mjd"])) 
...
clipped_data_c["Time"] = c_time_c
clipped_data_c["Flux"] = c_flux_c
clipped_data_c["STDEV"] = c_dev_c
...
plt.errorbar(clipped_data_o["Time"], clipped_data_o["Flux"], yerr=clipped_data_o["STDEV"], fmt='o', color='orange', alpha = 0.2, capsize=5)
plt.errorbar(clipped_data_c["Time"], clipped_data_c["Flux"], yerr=clipped_data_c["STDEV"], fmt='o', color='cyan', alpha = 0.2, capsize=5)
